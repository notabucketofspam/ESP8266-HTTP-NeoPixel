...
/*
 * Useful for getting the number of data chunks needed to receive a struct, for example.
 * Data chunks are always sixteen uint32_t's, thus 64-bytes long
 */
#define NP_DATA_CHUNK_COUNT(x) ((sizeof(x) + 63) / 64)
/*
 * Used for synchronizing between the GPIO interrupt and the main SPI read task
 */
  ...
  if (event == SPI_TRANS_DONE_EVENT) {
    ...
    if (transfer_done & SPI_SLV_WR_STA_DONE) {
      ...
      if (status == 0x00) {
        ...
        // Theoretically the hspi_slave_logic_device has been collecting stuff this whole time
        xSemaphoreGiveFromISR(spi_semaphore_handle, &xHigherPriorityTaskWoken);
      } else {
        // Set the global variable for the number of data chunks received
        data_chunk_count_length = status / 64;
      }
    }
...
void np_setup_spi() {
  ...
  spi_init(HSPI_HOST, &spi_config);
    ...
    uint32_t rx_buffer_size = (NP_DATA_CHUNK_COUNT(struct np_message_metadata) * 64) + CONFIG_NP_STREAM_BUFFER_SIZE;
  #else
    // Same idea but with only the metadata plus pattern data
    uint32_t rx_buffer_size = (NP_DATA_CHUNK_COUNT(struct np_message_metadata) * 64) +
      (NP_DATA_CHUNK_COUNT(struct np_pattern_data) * 64);
  #endif
  // tx_buffer_size is 64-bytes because it's unused at the moment and I don't know what else to do with it
  hspi_slave_logic_device_create(CONFIG_NP_SPI_HANDSHAKE, rx_buffer_size, 64, rx_buffer_size);
}
void IRAM_ATTR np_spi_slave_read_task(void *arg) {
  ...
  for (;;) {
    ...
    xSemaphoreTake(spi_semaphore_handle, portMAX_DELAY); // Block until more SPI data chunks are received
    ...
    for (uint32_t data_index_metadata = 0; data_index_metadata < NP_DATA_CHUNK_COUNT(struct np_message_metadata);
      ++data_index_metadata) {
      hspi_slave_logic_read_data((uint8_t *) data_chunk, 64, portMAX_DELAY);
